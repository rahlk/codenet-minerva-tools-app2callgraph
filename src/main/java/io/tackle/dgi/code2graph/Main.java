...
public class Main {
  ...
  private static void run(CommandLine cmd) throws Exception {
    ...
    try {
        ...
        Log.info("Finished construction of call graph. Took " + Long.toString(end_time - start_time) + " milliseconds.");

        // Save call graph as json
        CallGraphUtil.callgraph2Json(callGraph);
